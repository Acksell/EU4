...
def data_from_startpoint(text, pattern):
	return text[text.index(pattern):] if pattern in text else ''
...
def get_players_countries(save_txt):
	pattern = 'players_countries={'
	pc=get_bracket_content(data_from_startpoint(save_txt, pattern),indent_level=0)
	if pc:
